...
const WishlistedItem = ({ reload, product, user_id }) => {
  ...
  const removeFromWishlist = async () => {
    ...
    const res = await axios.post(
      `${REACT_APP_HOST}/remove-from-wishlist/${user_id}`,
  ...
  const AddToCartlist = async () => {
    ...
    _setLoading(true);
    const res = await axios.post(
      `${REACT_APP_HOST}/add-to-cart/${user_id}`,
      { product_id: product.product_id }
    );
    ...
    <div className="bg-white h-fit rounded-md p-5 mb-5 relative flex justify-between hover:shadow-md">
      ...
      {loading ? (
        ...
        <div className="btn-section w-2/12 h-44 flex flex-col justify-around">
          <button className="border-2 border-primary bg-primary py-2 mx-10 rounded-md font-semibold text-ternary-dark"
