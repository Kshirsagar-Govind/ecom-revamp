...
import { getPurchasedProducts } from "../lib/features/purchasedReducer";
import PurchasedListItem from "../components/Product/PurchasedlistItem";

const PurchasedListView = () => {
  const { purchasedList, isLoading } = useSelector(
    (state) => state.purchasedReducer
  );
  ...
  const getData = async () => {
      dispatch(getPurchasedProducts(userData.user_id));
  };
  ...
  if (isLoading) {
    ...
    return (
      <div className="w-full h-full p-2 px-12 mt-24">
        <div className="mb-4">
        <h1 className="text-xl text-center text-primary-dark font-semibold">Your Purchased History</h1>
        </div>
        {userData && purchasedList.length > 0 ? (
